...
export const STAT_SLUGS = ["hp", "attack", "defense", "special-attack", "special-defense", "speed"] as const;
  ...
  const [generations, types, chainList] = await Promise.all([
    ...
    Promise.all(
      TYPE_SLUGS.map(async (type) => {
        ...
        return {
          type,
          pokemon: response.pokemon.map((entry) => ({ id: idFromUrl(entry.pokemon.url), slot: entry.slot })),
        };
      }),
    ),
    listEvolutionChains(),
...
const STAT_SLUG_SET = new Set<string>(["hp", "attack", "defense", "special-attack", "special-defense", "speed"]);
  ...
  try {
    ...
    return {
      ...
      types: pokemon.types
        ...
        .filter(isTypeSlug),
      imageUrl:
        pokemon.sprites.other?.["official-artwork"]?.front_default ?? officialArtworkUrl(pokemon.id),
      stats: pokemon.stats
...
@keyframes sensor-pulse {
  0%, 100% { box-shadow: 0 0 12px 2px rgba(56, 189, 248, 0.7); }
  50% { box-shadow: 0 0 22px 6px rgba(56, 189, 248, 0.35); }
}
...
export function PokedexExplorer({ index }: { index: PokemonSummary[] }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <SearchBox key={filters.query === "" ? "empty" : "set"} initialQuery={filters.query} onQueryChange={setQuery} />
      <FilterBar
        filters={filters}
...
function Member({ member, isCurrent, currentLabel }: { member: EvolutionStageMember; isCurrent: boolean; currentLabel: string }) {
  const card = (
    <div
